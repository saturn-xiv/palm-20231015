...
use palm::{
    ...
    v1::{EmailTask, IdRequest, MinioProfile, Pager, Pagination, SmtpProfile},
...
impl v1::site_server::Site for Service {
            ...
            Some(ref req) => {
                ...
                try_grpc!(db.transaction::<_, Error, _>(move |db| {
                    ...
                    LogDao::add::<_, User>(
                        db,
                        user.id,
                        user_logs_response::item::Level::Info as i32,
                        &ss.client_ip,
                        Some(user.id),
                        "sign up.",
                    )?;
    ...
    async fn set_minio(&self, req: Request<MinioProfile>) -> GrpcResult<()> {
        let ss = Session::new(&req);
        let mut db = try_grpc!(self.pgsql.get())?;
        let db = db.deref_mut();
        let mut ch = try_grpc!(self.redis.get())?;
        ...
        if !user.is_administrator() {
            return Err(Status::permission_denied(type_name::<MinioProfile>()));
        }
        let req = req.into_inner();
    ...
    async fn get_minio(&self, req: Request<()>) -> GrpcResult<MinioProfile> {
        let ss = Session::new(&req);
        let mut db = try_grpc!(self.pgsql.get())?;
        let db = db.deref_mut();
        let mut ch = try_grpc!(self.redis.get())?;
        ...
        let mut it = try_grpc!(get::<MinioProfile, Aes>(db, aes, None))?;
    ...
    async fn set_smtp(&self, req: Request<SmtpProfile>) -> GrpcResult<()> {
        let ss = Session::new(&req);
        let mut db = try_grpc!(self.pgsql.get())?;
        let db = db.deref_mut();
        let mut ch = try_grpc!(self.redis.get())?;
        ...
        if !user.is_administrator() {
            return Err(Status::permission_denied(type_name::<SmtpProfile>()));
        }
        let req = req.into_inner();
    ...
    async fn get_smtp(&self, req: Request<()>) -> GrpcResult<SmtpProfile> {
        let ss = Session::new(&req);
        let mut db = try_grpc!(self.pgsql.get())?;
        let db = db.deref_mut();
        let mut ch = try_grpc!(self.redis.get())?;
        ...
        let mut it = try_grpc!(get::<SmtpProfile, Aes>(db, aes, None))?;
    ...
    async fn test_smtp(&self, req: Request<v1::SiteSmtpTestRequst>) -> GrpcResult<()> {
        ...
        let it = try_grpc!(get::<SmtpProfile, Aes>(db, aes, None))?;
        let task = EmailTask {
    ...
    async fn destroy_leave_word(&self, req: Request<IdRequest>) -> GrpcResult<()> {
        let ss = Session::new(&req);
        let mut db = try_grpc!(self.pgsql.get())?;
        let db = db.deref_mut();
        let mut ch = try_grpc!(self.redis.get())?;
