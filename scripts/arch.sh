...
build_gnu() {
    local target=$WORKSPACE/build/palm-arch-$1-$2
    mkdir -p $target
    cd $target
    ...
    cmake $WORKSPACE -DCMAKE_MAKE_PROGRAM=make -DCMAKE_BUILD_TYPE=$2 \
        ...
        -DVCPKG_HOST_TRIPLET=x64-linux -DVCPKG_TARGET_TRIPLET=$1-linux -DCMAKE_TOOLCHAIN_FILE=$HOME/local/vcpkg/scripts/buildsystems/vcpkg.cmake
