...
import { Dmz, RouterSetDmzRequest } from "../../protocols/ops-router_pb";
...
const Widget = ({ interfaces, dmz, refresh, setMessage }: IProps) => {
  const intl = useIntl();

  const formik = useFormik<IFormInput>({
    initialValues: {
      device: dmz?.getDevice() || "",
      mac: dmz?.getMac() || "",
      metric: dmz?.getMetric() || 200,
      address: dmz?.getAddress() || "",
      enable: dmz !== undefined,
    },
    validationSchema: formInputSchema,
    enableReinitialize: true,
    onSubmit: (values) => {
      const client = new RouterClient(GRPC_HOST);
      const request = new RouterSetDmzRequest();
      ...
      client.setDmz(request, grpc_metadata(), function (err, response) {
        if (err) {
  ...
  // const update_form = () => {
  //   formik.setFieldValue("device", dmz?.getDevice() || "");
  //   formik.setFieldValue("mac", dmz?.getMac() || "");
  //   formik.setFieldValue("metric", dmz?.getMetric() || "");
  //   formik.setFieldValue("address", dmz?.getAddress() || "");
  //   formik.setFieldValue("enable", dmz !== undefined);
  // };
