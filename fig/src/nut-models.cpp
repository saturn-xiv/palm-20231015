...
void palm::nut::dao::locale::create(soci::session& db, const std::string& lang,
...
std::vector<palm::nut::dao::category::Item> palm::nut::dao::category::parents(
    soci::session& db, const int32_t id) {
  const auto it = palm::nut::dao::category::get(db, id).value();
  soci::rowset<palm::nut::dao::category::Item> rs =
      (db.prepare << PARENTS, soci::use(it.left, palm::nut::dao::LEFT_COLUMN),
       soci::use(it.right, palm::nut::dao::RIGHT_COLUMN));
  std::vector<palm::nut::dao::category::Item> items(rs.begin(), rs.end());
  return items;
}
...
std::optional<palm::nut::dao::category::Item> palm::nut::dao::category::get(
  ...
  boost::optional<palm::nut::dao::category::Item> it;
  db << BY_CODE, soci::use(code, palm::nut::dao::CODE_COLUMN), soci::into(it);
  if (it.has_value()) {
    return it.get();
  }
  return std::nullopt;
...
void palm::nut::dao::shorter_link::update(soci::session& db, const int32_t id,
                                          const std::string& url,
                                          const std::string& details) {
  db << UPDATE, soci::use(id, palm::nut::dao::ID_COLUMN),
...
palm::nut::dao::shorter_link::get(soci::session& db, const int32_t offset,
  ...
  soci::rowset<palm::nut::dao::shorter_link::Item> rs =
      (db.prepare << ALL, soci::use(offset, palm::nut::dao::OFFSET_COLUMN),
       soci::use(limit, palm::nut::dao::LIMIT_COLUMN));
  std::vector<palm::nut::dao::shorter_link::Item> items(rs.begin(), rs.end());
  return items;
}
...
int32_t palm::nut::dao::log::total(soci::session& db, const int32_t user) {
  int32_t it;
  db << TOTAL, soci::into(it), soci::use(user, palm::nut::dao::USER_ID_COLUMN);
  return it;
}
std::vector<palm::nut::dao::log::Item> palm::nut::dao::log::get(
  ...
  soci::rowset<palm::nut::dao::log::Item> rs =
      (db.prepare << BY_USER, soci::use(user, palm::nut::dao::USER_ID_COLUMN),
...
std::optional<palm::nut::dao::role::user::Item> palm::nut::dao::role::user::get(
...
palm::nut::dao::role::user::by_role(soci::session& db, const int32_t role) {
  soci::rowset<palm::nut::dao::role::user::Item> rs =
      (db.prepare << BY_ROLE, soci::use(role, palm::nut::dao::ROLE_ID_COLUMN));
...
void palm::nut::dao::leave_word::create(soci::session& db,
                                        const std::string& lang,
                                        const std::string& ip,
                                        const v1::MediaContent::Editor editor,
...
std::vector<palm::nut::dao::leave_word::Item> palm::nut::dao::leave_word::get(
    soci::session& db, const int32_t offset, const int32_t limit) {
...
std::optional<palm::nut::dao::permission::Item> palm::nut::dao::permission::get(
...
void palm::nut::dao::attachment::create(soci::session& db, const int32_t user,
...
std::optional<palm::nut::dao::attachment::Item> palm::nut::dao::attachment::get(
    soci::session& db, const int32_t id) {
...
std::vector<palm::nut::dao::attachment::Item> palm::nut::dao::attachment::get(
    soci::session& db, const int32_t offset, const int32_t limit) {
  soci::rowset<palm::nut::dao::attachment::Item> rs = (db.prepare << BY_USER);
...
std::vector<palm::nut::dao::role::Item> palm::nut::dao::implicit_roles_for_user(
    soci::session& db, const int32_t id) {
  ...
  if (user->is_available()) {
    for (auto& iru : palm::nut::dao::role::user::by_user(db, id)) {
      if (iru.is_available()) {
        ...
        {
          const auto children = palm::nut::dao::role::offsprings(db, role->id);
          items.insert(std::end(items), std::begin(children),
                       std::end(children));
        }
      }
    }
...
std::vector<palm::nut::dao::user::Item> palm::nut::dao::implicit_users_for_role(
    soci::session& db, const int32_t id) {
