...
palm::fig::application::Command palm::fig::application::Main::parse(
  ...
  generic.add_options()("version,v", "print version string")(
      ...
      R"(command to execute, could be user-set-password, user-apply-role, i18n-sync, db-status, cache-list, cache-clear, web, rpc, worker)")(
  ...
  apply_role_to_user_desc.add_options()(
      "nickname,n", boost::program_options::value<std::string>(),
      "user's nickname")("role,r", boost::program_options::value<std::string>(),
                         "role's nickname");
...
void palm::fig::application::Main::launch(const Command& command) const {
  struct {
    ...
    void operator()(palm::fig::application::DbStatusCommand it) { it.start(); }
    void operator()(palm::fig::application::CacheListCommand it) { it.start(); }
    void operator()(palm::fig::application::CacheClearCommand it) {
      it.start();
    }
...
void palm::fig::application::I18nSyncCommand::start() const {
  ...
  auto postgresql = palm::fig::application::GenericOptions::postgresql(config);
...
void palm::fig::application::VersionCommand::start() const {
  std::cout << palm::GIT_VERSION << "(" << palm::BUILD_TIME << ")" << std::endl;
}
...
void palm::fig::application::RpcCommand::start() const {
  ...
  auto postgresql = palm::fig::application::GenericOptions::postgresql(config);
  auto redis = palm::fig::application::GenericOptions::redis(config);
  auto jwt = palm::fig::application::GenericOptions::jwt(config);
  const auto secret_box =
      palm::fig::application::GenericOptions::secret_box(config);
  auto opensearch = palm::fig::application::GenericOptions::opensearch(config);
  { opensearch->info(); }
  std::shared_ptr<palm::I18n> i18n =
      std::make_shared<palm::I18n>(postgresql, redis);

  palm::nut::services::User nut_user(postgresql, redis, jwt, secret_box, i18n,
                                     rabbitmq);
  ...
  palm::nut::services::Site nut_site(postgresql, redis, jwt, secret_box, i18n,
                                     rabbitmq, opensearch);
...
palm::fig::application::GenericOptions::secret_box(const toml::table& config) {
  ...
  std::shared_ptr<palm::SecretBox> it = std::make_shared<palm::SecretBox>(key);
  return it;
}
