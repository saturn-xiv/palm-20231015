...
import { Col, Button, Tooltip, message, Popconfirm, Typography } from 'antd';
...
import { Pager, IdRequest } from '@/protocols/nut_pb';
import { SiteClient } from '@/protocols/NutServiceClientPb';
import { GRPC_HOST, grpc_metadata } from '@/request';
import { ShowTimestamp } from '@/components/date';

interface ILeaveWordRemoveDialogProps {
  id: number;
  handleRefresh: () => void;
}

const LeaveWordRemoveDialog = (props: ILeaveWordRemoveDialogProps) => {
  const intl = useIntl();
  return (
    <Popconfirm
...
const LeaveWordsTable = () => {
  const intl = useIntl();
  const ref = useRef<ActionType>();
  return (
    <ProTable<ILeaveWordItem>
      search={false}
      actionRef={ref}
      columns={[
        {
