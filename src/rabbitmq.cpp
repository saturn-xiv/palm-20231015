...
inline static std::string amqp_bytes_to_string(const amqp_bytes_t& buf) {
  const std::string it((char*)buf.bytes, (char*)buf.bytes + buf.len);
  return it;
}
...
palm::rabbitmq::Connection::Connection(const std::string& host,
  ...
  spdlog::debug("open rabbitmq amqp://{}@{}:{}/{}?channel={}&version={}", user,
                host, port, virtual_host, this->_channel_id,
                AMQP_VERSION_STRING);
  ...
  {
    const auto reply =
        amqp_login(this->_con, virtual_host.c_str(), 0, 131072, 0,
  ...
  for (;;) {
    ...
    if ((envelope.message.properties._flags & AMQP_BASIC_CONTENT_TYPE_FLAG) &&
      ...
      const auto content_type =
          amqp_bytes_to_string(envelope.message.properties.content_type);
      const auto message_id =
          amqp_bytes_to_string(envelope.message.properties.message_id);
      const auto body = amqp_bytes_to_string(envelope.message.body);

      handler.execute(message_id, content_type, body);
    } else {
      spdlog::error("bad rabbitmq message");
    }
...
void palm::rabbitmq::Connection::declare_queue(const std::string& exchange_name,
  ...
  spdlog::debug("declare rabbitmq exchange ({}) with type({})", exchange_name,
                exchange_type);
  ...
  spdlog::debug("declare rabbitmq queue {}", queue);
  amqp_queue_declare(this->_con, this->_channel_id,
                     amqp_cstring_bytes(queue.c_str()), 0, (durable ? 1 : 0),
                     (exclusive ? 1 : 0), (durable ? 0 : 1), amqp_empty_table);
  ...
  spdlog::debug("bind rabbitmq queue({}) with routing-key({}) to exchange({})",
                queue, routing_key, exchange_name);
...
void palm::rabbitmq::Connection::declare_queue(const std::string& queue,
  ...
  amqp_queue_declare_ok_t* qd = amqp_queue_declare(
      this->_con, this->_channel_id, amqp_cstring_bytes(queue.c_str()), 0,
      (durable ? 1 : 0), 1, (durable ? 0 : 1), amqp_empty_table);
...
void palm::rabbitmq::Connection::publish(const std::string& exchange,
  ...
  spdlog::info("publish message({}) into ({}, {})", id, exchange, routing_key);
