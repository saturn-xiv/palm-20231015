...
class Connection {
 public:
  ...
  void produce(const T& payload) {
    ...
    this->produce(queue, id, palm::http::content_type::GRPC, buf);
  }
  ...
  void publish(const T& payload) {
    ...
    this->publish(queue, id, palm::http::content_type::GRPC, buf);
  }

  inline void declare_queue(const std::string& exchange,
                            const std::string& queue, bool durable = true,
                            bool exclusive = false) {
    this->declare_queue(exchange, "fanout", queue, "", durable, exclusive);
  }
  void declare_queue(const std::string& queue, const bool durable = false);

 private:
  void declare_queue(const std::string& exchange_name,
                     const std::string& exchange_type, const std::string& queue,
                     const std::string& routing_key, bool durable,
                     bool exclusive);
